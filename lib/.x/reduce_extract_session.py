...
def main(separator='\t'):
    ...
    #   current_key - string containing the concatenated key 
    #   (user_id, session_id)
    ...
    for current_key, group in groupby(data, itemgetter(0)):
        try:
            ...
            # Loop through the items in group and 
            # extract created_at and action type
            for i in group:
                ...
                print '%s\t%s\t%s\t%s' % (user_id, created_at, action, session_id)
